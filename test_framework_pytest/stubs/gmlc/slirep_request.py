...
def request(uri,method,ContentType,usr,pwd,postdata):
	...
	logging.basicConfig(filename="/tmp/log/gmlc/server_slirep.log", level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
	usrPass = usr + ":" + pwd
...
msid = sys.argv[2]
uri = sys.argv[3]
logging.basicConfig(filename="/tmp/server_slirep.log", level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
