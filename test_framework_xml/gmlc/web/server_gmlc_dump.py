...
#slirep_uri = config.get('web_mock', 'slirep_uri')
f=open(testdir + 'web/slia.dat')
slia_lines=f.readlines()
slia_line = 0
...
class root:      
	...
	def POST(self):
		logging.basicConfig(filename=testdir + "/logs/server_gmlc.log", level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')	
		...
		if (res_type == "ASYNC"):
			...
			if (slirep == "true"):
				os.system("sleep 1 && " + testdir + "/web/slirep_request.py " + testdir + " " + msid + " " + slirep_uri + " &")

		else:
			if (globals()['slia_line'] == len(slia_lines)):
				globals()['slia_line'] = 0	
			slia = slia_lines[slia_line]
			globals()['slia_line'] += 1
			logging.debug("slia:" + str(slia))		
			#logging.debug("slia_lines:" + str(len(slia_lines)))		
			logging.debug("msid:" + str(msid))		
			#slia_data = re.sub('#{' + poolvalue + '}', data, slia_data)
			slia = re.sub(r'<msid type="MSISDN">([0-9]+)</msid>', r'<msid type="MSISDN">' + msid + '</msid>', slia)
			#line = re.sub(r'\* \[(.*)\]\(#(.*)\)', r'<h2 id="\2">\1</h2>', line.rstrip())

			slia_2 = '''<!DOCTYPE svc_result PUBLIC "-//OMA//DTD SVC_RESULT 3.2//EN" "SVC_RESULT.DTD">
